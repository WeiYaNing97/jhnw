    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['share:switcherror:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['share:switcherror:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['share:switcherror:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['share:switcherror:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="switcherrorList" @selection-change="handleSelectionChange">
...
<script>
import { listSwitcherror, getSwitcherror, delSwitcherror, addSwitcherror, updateSwitcherror, exportSwitcherror } from "@/api/share/switcherror";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSwitcherror(this.queryParams).then(response => {
        this.switcherrorList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSwitcherror(errorId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.errorId != null) {
            updateSwitcherror(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSwitcherror(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除交换机错误编号为"' + errorIds + '"的数据项？').then(function() {
        return delSwitcherror(errorIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有交换机错误数据项？').then(() => {
        ...
        return exportSwitcherror(queryParams);
      }).then(response => {
