    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['share:switch_failure:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['share:switch_failure:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['share:switch_failure:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['share:switch_failure:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="switch_failureList" @selection-change="handleSelectionChange">
...
<script>
import { listSwitch_failure, getSwitch_failure, delSwitch_failure, addSwitch_failure, updateSwitch_failure, exportSwitch_failure } from "@/api/share/switch_failure";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSwitch_failure(this.queryParams).then(response => {
        this.switch_failureList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSwitch_failure(failureId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.failureId != null) {
            updateSwitch_failure(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSwitch_failure(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除交换机故障编号为"' + failureIds + '"的数据项？').then(function() {
        return delSwitch_failure(failureIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有交换机故障数据项？').then(() => {
        ...
        return exportSwitch_failure(queryParams);
      }).then(response => {
