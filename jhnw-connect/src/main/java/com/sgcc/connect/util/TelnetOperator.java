...
public class TelnetOperator {
    ...
    public String readUntil(String pattern) {
        ...
        try {
            ...
            while ((code = in.read()) != -1) {
                ...
                //匹配到结束标识时返回结果
                if (flag) {
                    if (ch == lastChar && sb.toString().endsWith(pattern) || sb.toString().trim().indexOf("---- More ----") !=-1) {
                        return sb.toString();
                    }
                }else{
                    //如果没指定结束标识,匹配到默认结束标识字符时返回结果
                    if(ch == promptChar || sb.toString().trim().indexOf("---- More ----") !=-1)
                        return sb.toString();
                }
                //登录失败时返回结果
     ...
     * @return
     */
    public String sendCommandAll(String command) {
        String return_command = sendCommand(command);
        String return_SUM = return_command;
        while (return_command.indexOf("---- More ----") != -1){
            return_command = sendCommand((char)32+"");
            return_SUM = return_SUM +" "+return_command;
        }
        return return_SUM;
    }
