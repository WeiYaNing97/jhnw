...
public class LuminousAttenuation {
    ...
    public AjaxResult obtainLightDecay(SwitchParameters switchParameters) {
        ...
        // 检查线程中断标志
        if (lightAttenuationParameters == null && WorkThreadMonitor.getShutdown_Flag(switchParameters.getScanMark())){
            // 如果线程中断标志为true，则直接返回
            return AjaxResult.success("操作成功", "线程已终止扫描");
        }
    ...
    public  AjaxResult getLightAttenuationCommandPojo(SwitchParameters switchParameters) {
        ...
        /**
         * 1: 根据品牌、型号、固件版本、子版本号 编写SQL语句 查询数据库光衰命令对象集合
         */
    ...
    public AjaxResult getPort(SwitchParameters switchParameters,LightAttenuationCommand lightAttenuationCommand) {
        ...
        /**
         * 1：提取lightAttenuationCommand对象中的 获取端口号命令
         *    发送获取交换机端口号命令，接受交换机返回信息集合
         *    判断交换机返回结果是否为null，如果为null则进行中断检查
         *    判断交换机返回结果是否为空，如果为空则进行告警并日志写入，然后返回错误结果
         */
        ...
        /**
         * 2：根据交换机返回信息集合获取获取 up状态 光衰端口号 铜缆除外，铜缆关键词为 COPPER
         *      判断光衰端口号port 是否为空
         */
        List<String> port = ObtainUPStatusPortNumber(switchParameters ,returnString_List);
        // 检查线程中断标志
        if (port == null && WorkThreadMonitor.getShutdown_Flag(switchParameters.getScanMark())){
            // 如果线程中断标志为true，则直接返回
            return null;
        }
        //判断光衰端口号port 是否为空
        ...
        /**
         * 3：查看是否需要转义：
         *    例如：GE转译为GigabitEthernet 才能执行获取交换机端口号光衰参数命令
         */
        String conversion = lightAttenuationCommand.getConversion();
        if (conversion != null){
            /*GE:GigabitEthernet;GE:GigabitEthernet  多个转移规则使用;分割*/
    ...
    public AjaxResult getLightAttenuationParameters(List<String> port, SwitchParameters switchParameters, LightAttenuationCommand lightAttenuationCommand) {
        ...
        /**
         * 1： 调用 getparameter 方法获取参数，并将结果保存在 getparameter 变量中
          */
        HashMap<String, String> getparameter = getparameter(port, switchParameters, lightAttenuationCommand.getGetParameterCommand());
        /**
         * 2：判断 getparameter 是否为空，如果为空则进行中断检查，
         * 如果为空集合则进行告警并日志写入，然后返回错误结果，否则返回成功结果的 AjaxResult 对象
         */
        if (getparameter == null && WorkThreadMonitor.getShutdown_Flag(switchParameters.getScanMark())){
            // 如果线程中断标志为true，则直接返回
            return null;
    ...
    /**
     * 根据up状态端口号及交换机获取光衰参数命令，发送命令并获取光衰参数信息
    ...
    public HashMap<String,String> getparameter(List<String> portNumber,SwitchParameters switchParameters,String command) {
        ...
        /**
         * 2：遍历端口号集合 结合数据库定义的光衰参数命令  获取交换机对应端口号的光衰参数命令，
         *    执行命令，并返回结果
         * */
        for (String port:portNumber){
            /**
             * 2.1：替换端口号 得到完整的 获取交换机对应端口号的光衰参数命令
            ...
            /**
             * 2.2：提取光衰参数
             *   判断返回结果是否为空，为空则跳过该端口号，继续下一个端口。
             *     否则获取光衰参数值并保存到hashMap中，
             */
            HashMap<String, Double> values =obtainTheParameterOfLightAttenuation( returnResults_List,switchParameters);
            // 检查线程中断标志
            if (values == null && WorkThreadMonitor.getShutdown_Flag(switchParameters.getScanMark())){
                // 如果线程中断标志为true，则直接返回
                return null;
     ...
     * 根据返回结果和交换机参数获取光衰参数值
     *
     * @param returnResults_List 返回结果字符串
    ...
    private HashMap<String, Double> obtainTheParameterOfLightAttenuation(List<String> returnResults_List, SwitchParameters switchParameters) {
        ...
        /**
         * 1：创建返回对象HashMap key为参数名称，value为对应的参数值，用于存储光衰参数值
         *      获取配置文件中光衰参数的配置信息，判断是否为空，为空则直接返回空HashMap
         *      不为空则获取key值, Key值来规定是按照表格的格式进行解析 还是按照字符串集合的格式进行解析
         *          Key值:R_table 按照表格的格式进行解析  ,  L_list 按照字符串集合的格式进行解析
         */
        HashMap<String, Double> values = new HashMap<>();
        // 获取关键字配置
        Map<String, Object> keywordS = FunctionalMethods.getKeywords(switchParameters,"光衰");
        ...
        /**
         * 2：根据keywordList判断是否为R_table或者L_list，进行处理后返回HashMap
         *    包含R_table 按照表格的格式进行解析  ,  包含L_list 按照字符串集合的格式进行解析
         */
        Map<String,String> keywordMap = new HashMap<>();
        if (keywordList.indexOf("R_table")!=-1){
            /**
             * 3：keywordList包含R_table 按照表格的格式进行解析
             *      获取R_table关键字配置，并解析表格数据
             *      关键字 TableHeader 为表格的表头
             *      关键字 RX 为表格的列名 提取完赋值给RX对象
             *      关键字 TX 为表格的列名 提取完赋值给RX对象
             */
            keywordMap = (Map<String,String>) keywordS.get("R_table");
            if (keywordMap == null){
                return values;
            }
            // 解析表格数据，集合是因为调用的方法考虑交换机返回信息中的表格位置可能不止一个，所以需要集合
            ...
            /**
             * 3：keywordList包含 L_list 按照列表的格式进行解析
             *      获取L_list关键字配置，并解析列表数据
             *       RX: 'Current Rx Power(dBM):$'
             *       TX: 'Current Tx Power(dBM):$'
             */
            keywordMap = (Map<String,String>) keywordS.get("L_list");
            if (keywordMap == null){
                return values;
            }
            // RX、TX
            ...
            for (String keywordName:strings){
                for (String keyword:returnResults_List) {
                    ...
                    if (mapvalue instanceof String){
                        ...
                        // 检查线程中断标志
                        if (theMeaningOfPlaceholders == null && WorkThreadMonitor.getShutdown_Flag(switchParameters.getScanMark())){
                            return null;
                        }
                        // RX 或 TX 都是有唯一值，所以判断长度应为1
        ...
        try {
            for (String portstr:ports){
                ...
                if (getparameter.get(portstr+"TX") != null && getparameter.get(portstr+"RX") != null){
                    // 根据交换机参数和端口号获取光衰比较对象，并插入或修改数据表数据，并返回插入结果和光衰比较对象*/
                    InsertLightAttenuation insertLightAttenuation = average(switchParameters,lightAttenuationComparisonMap, getparameter, portstr);
    ...
    */
     ...
     * @return 插入结果
     */
    public InsertLightAttenuation average(SwitchParameters switchParameters,
                                          Map<String,LightAttenuationComparison> lightAttenuationComparisonMap,
                                          HashMap<String,String> hashMap,String port) {

        /**1：根据交换机IP、四项基本信息以及端口号查询 上一步查询下胡来的数据库Map 记录*/
        LightAttenuationComparison lightAttenuationComparison = lightAttenuationComparisonMap.get(port);

        /**
         * 2：若光衰比较表列表为空则新插入信息,否则更新信息
         * */
        // 获取接收端光衰参数
        ...
        if (lightAttenuationComparison == null){
            /*需要新插入信息*/
